...
- Add a cloudflared remote provider (Windows x64, quick tunnel only) that needs no account, token, or DNS record: the pinned official client is downloaded and SHA-256 verified only after confirmation, launched with automatic updates disabled against the provider's plain HTTP loopback gateway, and its temporary `*.trycloudflare.com` hostname is validated before the authenticated gateway adopts it. Quick tunnels are temporary, rate-limited, and carry no uptime guarantee, so the panel and READMEs say so.
- Let the Android app classify `*.trycloudflare.com` as a supported remote tunnel host, so a scanned cloudflared pairing link selects remote access on its own instead of being rejected while no flow has been chosen yet. Requires an app build that includes this change; earlier builds still pair from the Remote access flow.
...
- Stop the quick-tunnel parser from adopting a reserved control-plane host: the banner can print `api.trycloudflare.com` before the tunnel's own hostname, and the pairing QR code would have pointed at Cloudflare's API instead of DSH.
- Add a Cloudflare **named** tunnel alongside the quick tunnel, so a cloudflared remote access can keep one stable address instead of a random one that changes on every reconnect. The panel takes a connector token, a public hostname, and a local forward port; the token is stored only in the DSH Mobile private directory, is passed to cloudflared through `TUNNEL_TOKEN` rather than the command line, and is never returned to any client. Cloudflare routes the hostname to the configured port, so that port is bound as configured and a taken port is reported as a hard failure instead of silently moving. Readiness comes from the connector's own `Registered tunnel connection` line, since a named tunnel prints no banner. Switching back to a quick tunnel removes the stored token.
